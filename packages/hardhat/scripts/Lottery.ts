...
async function startLottery(duration: string) {
  const contract = await viem.getContractAt("Lottery", contractAddress);
  const publicClient = await getClient();
  const currentBlock = await publicClient.getBlock();
  const timestamp = currentBlock?.timestamp ?? 0;
  const tx = await contract.write.startLottery([timestamp + BigInt(duration)]);
...
async function displayPrizePool(): Promise<string> {
  const contract = await viem.getContractAt("Lottery", contractAddress);
  
  const pool = await contract.read.prizePool()
 
  const prize = formatEther(pool);
  console.log(
    `Winner will collect ${prize} ${pool} Tokens\n`
  );
  return prize;
}

async function displayWinner(): Promise<string> {
  const contract = await viem.getContractAt("Lottery", contractAddress);
  
  const winner = await contract.read.winner()
 
  const prizeBN = await contract.read.prizeAmount([
    winner
  ]);
  const prize = formatEther(prizeBN);
  console.log(
    `The account of address ${winner} 
